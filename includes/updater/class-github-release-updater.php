<?php
/**
 * GitHub Release updater for My Blocks Launcher.
 *
 * The reusable logic is intentionally self-contained. When copying it to another
 * product, change the namespace and pass that product's own configuration.
 *
 * @package MyBlocksLauncher
 */
...
final class GitHub_Release_Updater {
    ...
    private function has_valid_configuration() {
        ...
        if ( ! $this->is_semantic_version( $this->config['version'] ) ) {
            return false;
        }

        if ( ! is_string( $this->config['update_uri'] ) ) {
            return false;
        }
        ...
        if ( 'plugin' === $this->config['type'] ) {
            if (
                ! is_string( $this->config['plugin_file'] ) ||
                dirname( $this->config['plugin_file'] ) !== $this->config['slug']
            ) {
                return false;
            }
        } elseif ( $this->config['stylesheet'] !== $this->config['slug'] ) {
            return false;
        }
    ...
    private function request_release() {
        ...
        if (
            ! empty( $body['draft'] ) ||
            ! empty( $body['prerelease'] ) ||
            ! isset( $body['tag_name'] ) ||
            ! is_string( $body['tag_name'] )
        ) {
            return null;
        }
